...
<style>
  .sa-ticket-link{text-decoration:none;color:inherit;display:block;border:1px solid var(--sa-line,#e2e8f0);border-radius:10px;padding:13px 14px;background:#fff;transition:.12s;}
  .sa-ticket-link:hover{border-color:var(--sa-accent,#c2f542);background:#fcfcfc;}
  .sa-thread{display:flex;flex-direction:column;gap:14px;}
  .sa-msg{display:flex;gap:12px;}
  .sa-msg.tenant{flex-direction:row;}
  .sa-msg.support{flex-direction:row-reverse;}
  .sa-msg .av{width:36px;height:36px;border-radius:50%;flex-shrink:0;display:grid;place-items:center;font-size:12px;font-weight:800;color:#fff;}
  .sa-msg.tenant .av{background:#0f172a;}
  .sa-msg.support .av{background:#6366f1;}
  .sa-msg.internal .av{background:#b45309;}
  .sa-msg .body{max-width:78%;flex:1;}
  .sa-msg.support .body, .sa-msg.internal .body{text-align:right;}
  .sa-msg .meta{font-size:11.5px;color:#64748b;display:inline-flex;gap:6px;align-items:center;margin-bottom:4px;flex-wrap:wrap;justify-content:flex-end;}
  .sa-msg.tenant .meta{justify-content:flex-start;}
  .sa-msg .bubble{display:inline-block;text-align:left;background:#f8fafc;border:1px solid #e2e8f0;border-radius:12px;padding:11px 14px;font-size:13.5px;line-height:1.6;color:#0f172a;white-space:pre-wrap;max-width:100%;}
  .sa-msg.support .bubble{background:#eef2ff;border-color:#c7d2fe;color:#312e81;}
  .sa-msg.internal .bubble{background:#fffbeb;border-color:#fcd34d;color:#78350f;font-style:italic;}
</style>
...
<div style="margin-bottom:16px;">
  ...
  <div style="background:#fff;border:1px solid var(--sa-line,#e2e8f0);border-radius:14px;padding:22px;margin-bottom:18px;">
    <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:16px;flex-wrap:wrap;margin-bottom:14px;">
      <div style="flex:1;min-width:280px;">
        <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-bottom:8px;">
          <span style="font-family:monospace;font-size:12px;font-weight:700;color:var(--sa-muted,#64748b);background:#f1f5f9;padding:3px 8px;border-radius:6px;">#T-<?php echo (int)$t['id']; ?></span>
        ...
        <h1 style="font-size:20px;font-weight:800;color:#0f172a;margin:0;line-height:1.3;"><?php echo htmlspecialchars($t['subject']); ?></h1>
      </div>
    </div>

    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:12px;padding:12px;background:#f8fafc;border-radius:10px;font-size:13px;">
      <div>
        <div style="font-size:11px;font-weight:700;text-transform:uppercase;color:#64748b;letter-spacing:0.4px;">From workspace</div>
        <?php if (!empty($t['tenant_name'])): ?>
          <div style="font-weight:700;color:#0f172a;margin-top:4px;">
            ...
            <?php if (!empty($t['tenant_public_id'])): ?><span style="font-family:monospace;font-size:11px;color:#94a3b8;margin-left:4px;"><?php echo htmlspecialchars($t['tenant_public_id']); ?></span><?php endif; ?>
          </div>
          <?php if (!empty($t['tenant_email'])): ?><div style="color:#64748b;font-size:12px;"><?php echo htmlspecialchars($t['tenant_email']); ?></div><?php endif; ?>
        <?php else: ?>
          <div style="color:#64748b;margin-top:4px;">Unknown (deleted tenant)</div>
        <?php endif; ?>
      </div>
      <?php if (!empty($t['branch_name'])): ?>
      <div>
        <div style="font-size:11px;font-weight:700;text-transform:uppercase;color:#64748b;letter-spacing:0.4px;">Related branch</div>
        <div style="font-weight:600;color:#0f172a;margin-top:4px;"><?php echo htmlspecialchars($t['branch_name']); ?></div>
      </div>
      <?php endif; ?>
      <div>
        <div style="font-size:11px;font-weight:700;text-transform:uppercase;color:#64748b;letter-spacing:0.4px;">Submitted by</div>
        <div style="font-weight:600;color:#0f172a;margin-top:4px;"><?php echo htmlspecialchars($t['submitted_by_name'] ?? 'Workspace owner'); ?></div>
        <div style="color:#94a3b8;font-size:11px;text-transform:capitalize;"><?php echo str_replace('_',' ',$t['submitted_by_kind'] ?? 'tenant'); ?></div>
      </div>
      <div>
        <div style="font-size:11px;font-weight:700;text-transform:uppercase;color:#64748b;letter-spacing:0.4px;">Assigned to</div>
        <form method="POST" action="support.php?id=<?php echo (int)$t['id']; ?>" style="margin-top:4px;">
          ...
          <select name="assign_to" onchange="this.form.submit()" style="padding:5px 8px;border-radius:6px;border:1px solid #cbd5e1;font-size:12.5px;background:#fff;min-width:130px;">
      ...
      <div>
        <div style="font-size:11px;font-weight:700;text-transform:uppercase;color:#64748b;letter-spacing:0.4px;">Opened</div>
        <div style="font-weight:600;color:#0f172a;margin-top:4px;"><?php echo sa_date($t['created_at'], 'M j, Y g:ia'); ?></div>
        <div style="color:#94a3b8;font-size:11px;"><?php echo sa_time_ago($t['created_at']); ?></div>
      </div>
    </div>
  </div>
  ...
  <!-- Thread -->
  <div style="background:#fff;border:1px solid var(--sa-line,#e2e8f0);border-radius:14px;padding:22px;margin-bottom:18px;">
    <h2 style="font-size:15px;font-weight:700;color:#0f172a;margin:0 0 18px;display:flex;align-items:center;gap:8px;">
    ...
    <div class="sa-thread">
      <?php if (empty($replies)): ?>
        <p style="color:#94a3b8;text-align:center;padding:20px;">No messages yet.</p>
      <?php else: ?>
        ...
        ?>
          <div class="sa-msg <?php echo $cls; ?>">
            ...
            <div class="body">
              <div class="meta">
                <strong style="color:#0f172a;font-size:12.5px;"><?php echo htmlspecialchars($r['author_name'] ?: $label); ?></strong>
                ...
                <span style="color:#94a3b8;"><?php echo sa_time_ago($r['created_at']); ?></span>
              </div>
  ...
  <!-- Reply form -->
  <div style="background:#fff;border:1px solid var(--sa-line,#e2e8f0);border-radius:14px;padding:20px;">
    <h3 style="font-size:14px;font-weight:700;color:#0f172a;margin:0 0 12px;">Reply</h3>
    <form method="POST" action="support.php?id=<?php echo (int)$t['id']; ?>">
      <?php echo sa_csrf_field(); ?>
      <input type="hidden" name="action" value="reply">
      <input type="hidden" name="ticket_id" value="<?php echo (int)$t['id']; ?>">
      <textarea name="message" rows="4" placeholder="Type your reply to the workspace..." style="width:100%;padding:12px;border-radius:8px;border:1px solid #cbd5e1;font-size:14px;font-family:inherit;resize:vertical;margin-bottom:10px;"></textarea>

      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:12px;align-items:end;">
        <div>
          <label style="display:block;font-size:12px;font-weight:700;color:#475569;margin-bottom:5px;">Change status</label>
          <select name="new_status" style="width:100%;padding:8px 10px;border-radius:7px;border:1px solid #cbd5e1;font-size:13.5px;background:#fff;">
...
<?php else: /* inbox list view */ ?>
  ...
  <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:16px;flex-wrap:wrap;margin-bottom:20px;">
    <div>
      <h1 style="font-size:22px;font-weight:800;margin:0 0 4px;">Support Inbox</h1>
    ...
    <?php if ($counts['urgent'] > 0): ?>
      <div style="background:#fef2f2;border:1px solid #fecaca;color:#991b1b;border-radius:10px;padding:10px 14px;font-size:13px;font-weight:700;display:flex;align-items:center;gap:8px;">
  ...
  <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:12px;margin-bottom:20px;">
    ...
    $kpis = [
      ...
      ['all',         'All tickets',    $counts['all'],         '#0f172a', false],
    ];
    foreach ($kpis as $k):
      $active = ($filter === $k[0]);
    ?>
      <a href="?filter=<?php echo $k[0]; ?>" style="text-decoration:none;background:#fff;border:1px solid <?php echo $active ? $k[3] : 'var(--sa-line,#e2e8f0)'; ?>;border-left:3px solid <?php echo $k[3]; ?>;border-radius:10px;padding:14px 16px;transition:.12s;">
        ...
        <div style="font-size:24px;font-weight:800;color:#0f172a;margin-top:4px;"><?php echo (int)$k[2]; ?></div>
      </a>
    <?php endforeach; ?>
  </div>

  <!-- Filter bar -->
  <form method="GET" action="support.php" style="background:#fff;border:1px solid var(--sa-line,#e2e8f0);border-radius:12px;padding:12px 14px;margin-bottom:16px;display:flex;gap:10px;flex-wrap:wrap;align-items:end;">
    <input type="hidden" name="filter" value="<?php echo htmlspecialchars($filter); ?>">
    <div style="flex:1;min-width:200px;">
      <label style="display:block;font-size:11px;font-weight:700;color:#64748b;margin-bottom:4px;text-transform:uppercase;letter-spacing:0.4px;">Search</label>
      <input type="search" name="q" value="<?php echo htmlspecialchars($search); ?>" placeholder="Subject, message, tenant name or email..." style="width:100%;padding:8px 10px;border-radius:7px;border:1px solid #cbd5e1;font-size:13px;">
    </div>
    <div>
      <label style="display:block;font-size:11px;font-weight:700;color:#64748b;margin-bottom:4px;text-transform:uppercase;letter-spacing:0.4px;">Priority</label>
      <select name="priority" style="padding:8px 10px;border-radius:7px;border:1px solid #cbd5e1;font-size:13px;background:#fff;">
    ...
    <div>
      <label style="display:block;font-size:11px;font-weight:700;color:#64748b;margin-bottom:4px;text-transform:uppercase;letter-spacing:0.4px;">Type</label>
      <select name="type" style="padding:8px 10px;border-radius:7px;border:1px solid #cbd5e1;font-size:13px;background:#fff;">
    ...
    <div>
      <label style="display:block;font-size:11px;font-weight:700;color:#64748b;margin-bottom:4px;text-transform:uppercase;letter-spacing:0.4px;">Assigned</label>
      <select name="assigned" style="padding:8px 10px;border-radius:7px;border:1px solid #cbd5e1;font-size:13px;background:#fff;">
    ...
    <?php if ($filter !== 'open' || $priority_f !== 'all' || $type_f !== 'all' || $assigned_f !== 'all' || $search !== ''): ?>
      <a href="support.php" style="padding:8px 14px;border-radius:7px;background:#f1f5f9;color:#475569;text-decoration:none;font-weight:600;font-size:13px;display:inline-flex;align-items:center;">Reset</a>
    <?php endif; ?>
  </form>

  <?php if (empty($tickets)): ?>
    <div style="background:#fff;border:1px solid var(--sa-line,#e2e8f0);border-radius:14px;padding:60px 20px;text-align:center;color:var(--sa-muted,#94a3b8);">
      <?php echo sa_icon('check-circle'); ?>
      <p style="font-size:15px;font-weight:700;color:#0f172a;margin:14px 0 6px;">Inbox zero! 🎉</p>
            ...
            <div style="flex:1;min-width:0;">
              <div style="display:flex;justify-content:space-between;gap:10px;flex-wrap:wrap;align-items:center;margin-bottom:4px;">
                <div style="display:flex;gap:6px;align-items:center;flex-wrap:wrap;">
                  <span style="font-family:monospace;font-size:11.5px;font-weight:700;color:#64748b;">#T-<?php echo (int)$t['id']; ?></span>
                ...
                <div style="display:flex;gap:8px;align-items:center;">
                  ...
                  <span style="font-size:11.5px;color:#94a3b8;white-space:nowrap;"><?php echo sa_time_ago($t['updated_at'] ?: $t['created_at']); ?></span>
                </div>
              </div>
              <h3 style="font-size:14px;font-weight:700;color:#0f172a;margin:0 0 3px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;"><?php echo htmlspecialchars($t['subject']); ?></h3>
              <div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;font-size:12px;color:#64748b;">
                <span style="font-weight:600;color:#334155;"><?php echo htmlspecialchars($t['tenant_name'] ?? 'Unknown tenant'); ?></span>
                <?php if (!empty($t['tenant_public_id'])): ?>
                  <span style="font-family:monospace;color:#94a3b8;"><?php echo htmlspecialchars($t['tenant_public_id']); ?></span>
                <?php endif; ?>
